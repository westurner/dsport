...
| ODT writer | `docutils.writers.odf_odt` | `src/docutilsrs/src/odt_writer.rs` | accepted-deviation | `docutilsrs.parse_to_odt` produces a valid `.odt` ZIP container (`mimetype` + `META-INF/manifest.xml` + `content.xml` + `styles.xml`) with a minimal `office:document-content` body covering sections/headings, paragraphs, inline emphasis/strong/literal, bullet/enumerated lists, definition + field lists, block quotes (`Quotations` style), preformatted text, tables with `number-columns-spanned`/`number-rows-spanned`, references (`text:a`), images (`draw:image`), and `raw:: odt` passthrough. Not byte-parity-gated against the upstream writer (which is ~3.5kLoC + a binary `styles.odt` template); structural parity is exercised by `tests/test_writer_odt.py`. |
