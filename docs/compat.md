...
| inline roles + inline math | `myst_parser.parsers.mdit.roles` / `mdit_py_plugins.dollarmath` (inline) | `src/myst-md-rs/src/role.rs` + `src/myst-md-rs/src/render.rs` | accepted-deviation | `{name}` backtick-run roles → `<span class="myst-role" data-role="…">`; `$…$` → `<span class="math">`; handles split across `Text` + `Code` event boundaries via cross-event `pending_role` state |
