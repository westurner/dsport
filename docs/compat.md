...
| rST parser — tables (simple + grid) | `docutils.parsers.rst.tableparser` | `src/docutilsrs/src/parser.rs` | accepted-deviation | head + body rows; column spans (`morecols`) supported for grid tables; row spans (`morerows`) and multi-paragraph cells still deferred |
...
| ODT writer | `docutils.writers.odf_odt` | — | pending | requires producing a `.odt` ZIP container (`content.xml`, `styles.xml`, `META-INF/manifest.xml`, `mimetype`); deferred |
