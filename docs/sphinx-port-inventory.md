...
| `cmd/build.py` + `cmd/make_mode.py` | `sphinxdocrs::build` | **C2** | **done** — arg parser, all `_parse_*` helpers, `jobs_argument`, `MakeMode` (`build_clean`, `build_help`, `run_generic_build`, full `BUILDERS` table, target dispatch) ported; 35 Rust-side integration tests; `sphinx-build -M` runs natively; `sphinx-build -b html` now runs natively via `SphinxApp`; other `-b` builders delegate to Python |
...
| `builders/` | `sphinxdocrs::builders` | **P3** | **partial** — `Builder` trait + `HtmlBuilder` (minimal path: `get_target_uri`, `build_doc`, `build_all`); RST → `docutilsrs::parse_rst` → `docutilsrs::html5` → wrapped HTML5 page → `.html` file; 19 inline tests + 22 integration tests in `tests/builders.rs`; theming/Jinja2/search/CSS/JS assets deferred |
...
| `test_application.py` | application | P3 | **partial** — `tests/application.rs` (17 tests): `NATIVE_BUILDERS`/`is_native_builder`, `SphinxApp::new` path validation (missing srcdir, identical paths, creates outdir), constructor fields, `supports_native`, `build()` HTML output (index.html, valid HTML5, RST content, multi-doc, unknown builder error), config defaults, config overrides; full `Sphinx()` Python-backed invocation deferred |
| `test_command_line.py`, `test__cli/` | cli | P3 | **partial** — arg-parsing layer ported natively (`build::parser`, `build::args`, `build::logging`, `build::make_mode`); `sphinx-build -b html` now native via `SphinxApp`; full `Sphinx()` Python invocation tests deferred |
...
| `test_builders/` | builders | P3 | **partial** — `tests/builders.rs` (22 tests): `Builder` trait assertions (`name`, `format`, `out_suffix`), `get_target_uri` (plain, subdir), `build_doc` (HTML5 structure, DOCTYPE, charset, section title, paragraph, bold/italic, bullet list, literal block, output path, subdirectory), `build_all` (empty, single, multiple, subdirectory, ignores non-rst, uses env.all_docs when populated); full Sphinx app tests deferred |
...
| **C2c** `sphinx-build -b` direct mode | `sphinxdocrs::application::SphinxApp` | **done (html)** — `SphinxApp` wires `SphinxConfig` + `SphinxComponentRegistry` + `BuildEnvironment` + `HtmlBuilder`; `sphinx-build -b html` runs natively; other builders still delegate to Python |
...
| `src/sphinxdocrs/src/builders/mod.rs` | `sphinx.builders.Builder` | `Builder` trait (`name`, `format`, `out_suffix`, `get_target_uri`, `build_doc`, `build_all`); `BuildError`, `BuildResult` |
| `src/sphinxdocrs/src/builders/html.rs` | `sphinx.builders.html.StandaloneHTMLBuilder` | `HtmlBuilder` |
| `src/sphinxdocrs/src/builders/latex.rs` | `sphinx.builders.latex.LaTeXBuilder` | `LatexBuilder`: RST → docutilsrs → `.tex` file |
...
| id | task | closes row | gate |
| --- | --- | --- | --- |
| **G7** ✅ | `html` builder minimal path: read env, run `docutilsrs::html5_writer`, emit pages + assets via registry CSS/JS plumbing (already present). | `builders/` → **partial** | `tests/builders.rs` (22 tests) |
| **G8** ✅ | Full `Sphinx` application wiring `Config` (G3) + `registry` (G4a) + `environment` (G4) + html builder (G7). Closes **C2c** native `sphinx-build -b html`. | `test_application.py` → **partial**; **C2c** → **done (html)** | `tests/application.rs` (17 tests) |
| **G9** ✅ | Remaining writers/builders one at a time: `latex` and `man` native via `LatexBuilder`/`ManpageBuilder` wrapping existing `docutilsrs` writers. | `builders/` (latex, man) → **partial** | inline tests in `builders/latex.rs`, `builders/manpage.rs` |
