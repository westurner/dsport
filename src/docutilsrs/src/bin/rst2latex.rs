...
#[command(
    name = "rst2latex",
    about = "Generate LaTeX documents from standalone reStructuredText sources.",
    help_template = "Usage\n=====
  {usage}\n\n{about}\n\nOptions\n=======\n{options}"
)]
struct Cli {
    ...
    #[command(flatten)]
    specific: docutilsrs::cli::LatexOptions,
}
...
fn main() {
    ...
    if py_passed {
        let status = std::process::Command::new(&python_bin)
            .arg("-c")
            .arg(format!("import docutils.core; import sys; sys.argv[0] = '{}'; sys.exit(docutils.core.{}())", "rst2latex", "rst2latex"))
            .args(&python_args)
            .status()
