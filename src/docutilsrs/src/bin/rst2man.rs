...
#[command(
    name = "rst2man",
    about = "Generate manual pages from standalone reStructuredText sources.",
    help_template = "Usage\n=====
  {usage}\n\n{about}\n\nOptions\n=======\n{options}"
)]
struct Cli {
    ...
    #[command(flatten)]
    specific: docutilsrs::cli::ManOptions,
}
...
fn main() {
    ...
    if py_passed {
        let status = std::process::Command::new(&python_bin)
            .arg("-c")
            .arg(format!("import docutils.core; import sys; sys.argv[0] = '{}'; sys.exit(docutils.core.{}())", "rst2man", "rst2man"))
            .args(&python_args)
            .status()
