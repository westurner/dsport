...
#[command(
    name = "rst2odt",
    about = "Generate OpenDocument text documents from standalone reStructuredText sources.",
    help_template = "Usage\n=====
  {usage}\n\n{about}\n\nOptions\n=======\n{options}"
)]
struct Cli {
...
fn main() {
    ...
    if py_passed {
        let status = std::process::Command::new(&python_bin)
            .arg("-c")
            .arg(format!("import docutils.core; import sys; sys.argv[0] = '{}'; sys.exit(docutils.core.{}())", "rst2odt", "rst2odt"))
            .args(&python_args)
            .status()
    ...
    let output = odt(&tree, &cli.specific, &cli.common);
