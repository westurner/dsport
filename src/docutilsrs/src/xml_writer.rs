...
/// Similar to `XMLTranslator.generator`'s `'<!-- Generated by Docutils %s -->\n'`
/// shape closely enough for `tests/parity.rs`'s generator-comment normalizer
/// (`(?is)<!--\s*(?:generated by|created using).*?-->`) to canonicalize both
/// sides identically; the exact version text doesn't matter since it's
/// always normalized away before comparison.
const GENERATOR_COMMENT: &str = "<!-- Generated by Docutilsrs -->\n";
...
mod tests {
    ...
    fn every_opened_tag_is_closed_in_stack_order() {
        ...
        // Skip the XML prolog, DOCTYPE, and generator-comment header lines.
        for line in xml.lines().skip(3) {
