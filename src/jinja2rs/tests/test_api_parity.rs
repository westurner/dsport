...
fn test_environment_creation_exact() {
    ...
    // Basic environment should be created without issues
    // features are at crate level, not instance level
    assert!(jinja2rs::features().len() > 0);
}
...
fn test_render_str_dictionary_access_exact() {
    let env = Environment::new();
    let tmpl = "{{ data['key'] }}";
    let result = env.render_str(tmpl, &json!({"data": {"key": "value"}})).unwrap();
...
fn test_render_str_list_indexing_exact() {
    ...
    let result = env.render_str(tmpl, &json!({"items": [10, 20, 30]})).unwrap();
...
fn test_render_str_string_operations_exact() {
    ...
    assert_eq!(env.render_str(tmpl, &json!({"text": "hello"})).unwrap(), "5");
