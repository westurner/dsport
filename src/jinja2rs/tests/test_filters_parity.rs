...
fn test_join_exact() {
    ...
    let tmpl = "{{ items | join(' - ') }}";
    let result = env.render_str(tmpl, &json!({"items": ["a", "b", "c"]})).unwrap();
...
fn test_indent_exact() {
    ...
    let result = env.render_str(tmpl, &json!({"text": "hello\nworld"})).unwrap();
    ...
    let result = env.render_str(tmpl, &json!({"text": "line1\nline2"})).unwrap();
...
fn test_wordwrap_exact() {
    ...
    let result = env.render_str(tmpl, &json!({"text": "this is a test"})).unwrap();
...
fn test_xmlattr_sphinx_specific() {
    ...
    let result = env.render_str(
        tmpl,
        &json!({"attrs": {"class": "active", "id": "header"}}),
    ).unwrap();
...
fn test_slice_index_sphinx_specific() {
    ...
    let result = env.render_str(tmpl, &json!({"items": [1, 2, 3, 4, 5, 6]})).unwrap();
    assert!(!result.is_empty());
}
...
fn test_min_exact() {
    ...
    let tmpl = "{{ items | min }}";
    let result = env.render_str(tmpl, &json!({"items": [3, 1, 4, 1, 5]})).unwrap();
    assert_eq!(result, "1");
}
...
fn test_sum_exact() {
    ...
    let tmpl = "{{ items | sum }}";
    let result = env.render_str(tmpl, &json!({"items": [1, 2, 3, 4]})).unwrap();
    assert_eq!(result, "10");
}
...
fn test_reject_exact() {
    ...
    let result = env.render_str(tmpl, &json!({"items": [1, 2, 3, 4, 5]})).unwrap();
