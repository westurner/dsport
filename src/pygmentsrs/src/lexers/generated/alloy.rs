...
fn build_table() -> Table {
    ...
    m.insert(r"fact", vec![
        ...
        Rule::token_to(r"(?ms)\{", OPERATOR, NewState::Pop(1)),
        Rule::token_to(r#"(?ms)[a-zA-Z_][\w]*"*"#, NAME, NewState::Pop(1)),
        Rule::token_to(r#"(?ms)"\b(\\\\|\\[^\\]|[^"\\])*""#, STRING, NewState::Pop(1)),
    ]);
    m.insert(r"root", vec![
        ...
        Rule::token(r"(?ms)[^\S\n]+", WHITESPACE),
        Rule::bygroups_to(r"(?ms)(module|open)(\s+)", vec![Some(KEYWORD_NAMESPACE), Some(WHITESPACE)], NewState::Push(vec![r"module"])),
        Rule::bygroups_to(r"(?ms)(sig|enum)(\s+)", vec![Some(KEYWORD_DECLARATION), Some(WHITESPACE)], NewState::Push(vec![r"sig"])),
        Rule::token(r"(?ms)(iden|univ|none)\b", KEYWORD_CONSTANT),
        Rule::token(r"(?ms)(int|Int)\b", KEYWORD_TYPE),
        Rule::token(r"(?ms)(var|this|abstract|extends|set|seq|one|lone|let)\b", KEYWORD),
        Rule::token(r"(?ms)(all|some|no|sum|disj|when|else)\b", KEYWORD),
        Rule::token(r"(?ms)(run|check|for|but|exactly|expect|as|steps)\b", KEYWORD),
        Rule::token(r"(?ms)(always|after|eventually|until|release)\b", KEYWORD),
        Rule::token(r"(?ms)(historically|before|once|since|triggered)\b", KEYWORD),
        Rule::token(r"(?ms)(and|or|implies|iff|in)\b", OPERATOR_WORD),
        Rule::bygroups_to(r"(?ms)(fun|pred|assert)(\s+)", vec![Some(KEYWORD), Some(WHITESPACE)], NewState::Push(vec![r"fun"])),
        Rule::bygroups_to(r"(?ms)(fact)(\s+)", vec![Some(KEYWORD), Some(WHITESPACE)], NewState::Push(vec![r"fact"])),
