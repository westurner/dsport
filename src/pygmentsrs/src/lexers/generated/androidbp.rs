...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        Rule::bygroups_to(r"(?m)(\w*)(\s*)(\+?=)(\s*)", vec![Some(NAME_VARIABLE), Some(WHITESPACE), Some(OPERATOR), Some(WHITESPACE)], NewState::Push(vec![r"assign-rhs"])),
        Rule::bygroups_to(r"(?m)(\w*)(\s*)(\{)", vec![Some(NAME_FUNCTION), Some(WHITESPACE), Some(PUNCTUATION)], NewState::Push(vec![r"in-rule"])),
    ...
    m.insert(r"in-map", vec![
        Rule::bygroups(r"(?m)(\w+)(:)(\s*)", vec![Some(NAME), Some(PUNCTUATION), Some(WHITESPACE)]),
        Rule::token(r"(?m)(true|false)\b", NAME_BUILTIN),
        Rule::token(r"(?m)0x[0-9a-fA-F]+", NUMBER_HEX),
        Rule::token(r"(?m)\d+", NUMBER_INTEGER),
        Rule::token(r#"(?m)".*?""#, STRING),
