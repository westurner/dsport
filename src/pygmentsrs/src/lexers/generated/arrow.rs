...
fn build_table() -> Table {
    ...
    m.insert(r"fparams", vec![
        Rule::bygroups(r"(?m)\b(int|bool|char)((?:\[\])*)(?=\s+)(\s+)([a-zA-Z_][a-zA-Z0-9_]*)", vec![Some(KEYWORD_TYPE), Some(PUNCTUATION), Some(WHITESPACE), Some(NAME_VARIABLE)]),
        Rule::token(r"(?m),", PUNCTUATION),
        Rule::token_to(r"(?m)\)", PUNCTUATION, NewState::Pop(1)),
    ]);
    m.insert(r"escape", vec![
        Rule::token(r#"(?m)\\(["\\/abfnrtv]|[0-9]{1,3}|x[0-9a-fA-F]{2}|u[0-9a-fA-F]{4})"#, STRING_ESCAPE),
    ]);
    m.insert(r"char", vec![
