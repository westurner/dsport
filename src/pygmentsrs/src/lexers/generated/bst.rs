...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        ...
        Rule::token_to(r"(?im)(execute|i(?:ntegers|terate)|reverse|strings)", KEYWORD, NewState::Push(vec![r"group"])),
        Rule::token_to(r"(?im)(function|macro)", KEYWORD, NewState::Push(vec![r"group", r"group"])),
        Rule::token_to(r"(?im)(entry)", KEYWORD, NewState::Push(vec![r"group", r"group", r"group"])),
    ]);
    m.insert(r"whitespace", vec![
        Rule::token(r"(?im)\s+", WHITESPACE),
        Rule::token(r"(?im)%.*?$", COMMENT_SINGLE),
    ]);
    m.insert(r"group", vec![
        Rule::token(r"(?im)\s+", WHITESPACE),
        Rule::token(r"(?im)%.*?$", COMMENT_SINGLE),
        Rule::token_to(r"(?im)\{", PUNCTUATION, NewState::Push(vec![r"#pop", r"group-end", r"body"])),
    ]);
    m.insert(r"group-end", vec![
        Rule::token(r"(?im)\s+", WHITESPACE),
        Rule::token(r"(?im)%.*?$", COMMENT_SINGLE),
    ...
    m.insert(r"body", vec![
        ...
        Rule::token_to(r"(?im)\{", PUNCTUATION, NewState::Push(vec![r"group-end", r"body"])),
