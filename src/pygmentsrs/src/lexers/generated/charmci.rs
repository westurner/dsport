//! AUTO-GENERATED from `pygments.pygments.lexers.c_like:CharmciLexer`.
//!
//! Do not edit by hand. Regenerate with:
//!   python tools/gen_lexer.py pygments.lexers.c_like:CharmciLexer:charmci
...
fn build_table() -> Table {
    ...
    m.insert(r"keywords", vec![
        Rule::bygroups_to(r"(?m)(module)(\s+)", vec![Some(KEYWORD), Some(TEXT)], NewState::Push(vec![r"classname"])),
        Rule::token(r"(?m)(array|c(?:hare|onditional)|group|m(?:(?:ain(?:char|modul)|essag)e)|nodegroup)", KEYWORD),
        Rule::token(r"(?m)(a(?:ccel(?:(?:block)?)|ggregate)|createh(?:(?:er|om)e)|e(?:ntry|x(?:clusive|pedited))|i(?:get|mmediate|n(?:it(?:call|node|proc)|line))|local|m(?:emcritical|(?:igra|u)table)|no(?:copy|keep|trace)|p(?:acked|ython)|re(?:ad(?:only|write)|ductiontarget)|s(?:tacksize|ync)|threaded|varsize|writeonly)", KEYWORD),
