...
fn build_table() -> Table {
    ...
    m.insert(r"module.name", vec![
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token_to(r"(?m)[a-zA-Z_][a-zA-Z0-9_.`]+", NAME_CLASS, NewState::Pop(1)),
    ]);
    m.insert(r"import.module", vec![
        Rule::bygroups(r"(?m)\b(qualified)\b(\s*)", vec![Some(KEYWORD), Some(WHITESPACE)]),
        Rule::bygroups_to(r"(?m)(\s*)\b(as)\b", vec![Some(WHITESPACE), Some(KEYWORD)], NewState::Push(vec![r"#pop", r"import.module.as"])),
    ...
    m.insert(r"import.module.as", vec![
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token_to(r"(?m)[a-z`][\w`]*", NAME_CLASS, NewState::Pop(1)),
        Rule::token_to(r"(?m)[A-Z`][\w`]*", NAME_CLASS, NewState::Pop(1)),
    ]);
    m.insert(r"import.what", vec![
        Rule::bygroups_to(r"(?m)\b(class)\b(\s+)([a-z`][\w`]*|[A-Z`][\w`]*|[~@#$%\^?!+\-*<>\\/|&=:]+)", vec![Some(KEYWORD), Some(WHITESPACE), Some(NAME_CLASS)], NewState::Push(vec![r"import.what.class"])),
        Rule::bygroups_to(r"(?m)\b(instance)(\s+)([a-z`][\w`]*|[A-Z`][\w`]*|[~@#$%\^?!+\-*<>\\/|&=:]+)(\s+)", vec![Some(KEYWORD), Some(WHITESPACE), Some(NAME_CLASS), Some(WHITESPACE)], NewState::Push(vec![r"import.what.instance"])),
        Rule::bygroups_to(r"(?m)(::)(\s*)\b([A-Z`][\w`]*)\b", vec![Some(PUNCTUATION), Some(WHITESPACE), Some(NAME_CLASS)], NewState::Push(vec![r"import.what.type"])),
        Rule::bygroups(r"(?m)\b(generic)\b(\s+)\b([a-z`][\w`]*|[A-Z`][\w`]*)\b", vec![Some(KEYWORD), Some(WHITESPACE), Some(NAME)]),
        Rule::token(r"(?m)[a-z`][\w`]*", NAME),
        Rule::token(r"(?m)_[a-z`][\w`]*", NAME),
        Rule::token(r"(?m)[~@#$%\^?!+\-*<>\\/|&=:]+", NAME_FUNCTION),
        Rule::token(r"(?m)[A-Z`][\w`]*", NAME_CLASS),
    ...
    m.insert(r"import.what.class", vec![
        Rule::token_to(r"(?m),", PUNCTUATION, NewState::Pop(1)),
        Rule::token_to(r"(?m)\(", PUNCTUATION, NewState::Push(vec![r"import.what.class.members"])),
        Rule::token_to(r"(?m)$", WHITESPACE, NewState::Pop(2)),
        Rule::token(r"(?m)\s+", WHITESPACE),
    ]);
    m.insert(r"import.what.class.members", vec![
        Rule::token(r"(?m),", PUNCTUATION),
        Rule::token(r"(?m)\.\.", PUNCTUATION),
        Rule::token_to(r"(?m)\)", PUNCTUATION, NewState::Pop(1)),
        Rule::token(r"(?m)[a-z`][\w`]*", NAME),
    ...
    m.insert(r"import.what.instance", vec![
        Rule::token_to(r"(?m)[,)]", PUNCTUATION, NewState::Pop(1)),
        Rule::token_to(r"(?m)\(", PUNCTUATION, NewState::Push(vec![r"import.what.instance"])),
        Rule::token_to(r"(?m)$", WHITESPACE, NewState::Pop(2)),
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token(r"(?m)[a-z`][\w`]*", NAME),
        Rule::token(r"(?m)_[a-z`][\w`]*", NAME),
    ...
    m.insert(r"import.what.type", vec![
        Rule::token_to(r"(?m),", PUNCTUATION, NewState::Pop(1)),
        Rule::token_to(r"(?m)[({]", PUNCTUATION, NewState::Push(vec![r"import.what.type.consesandfields"])),
        Rule::token_to(r"(?m)$", WHITESPACE, NewState::Pop(2)),
        Rule::token(r"(?m)\s+", WHITESPACE),
    ]);
    m.insert(r"import.what.type.consesandfields", vec![
        Rule::token(r"(?m),", PUNCTUATION),
        Rule::token(r"(?m)\.\.", PUNCTUATION),
        Rule::token_to(r"(?m)[)}]", PUNCTUATION, NewState::Pop(1)),
        Rule::token(r"(?m)[a-z`][\w`]*", NAME),
    ...
    m.insert(r"literals.stringd", vec![
        Rule::token(r#"(?m)[^\\"\n]+"#, STRING_DOUBLE),
