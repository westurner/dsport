...
//!   python tools/gen_lexer.py pygments.lexers.cplint:CplintLexer:cplint
...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        Rule::token(r"(?m)map_query", KEYWORD),
        Rule::token(r"(?m)(b(?:eta|inomial)|di(?:richlet|screte)|exponential|finite|g(?:a(?:mma|ussian)|eometric)|multinomial|p(?:ascal|oisson)|u(?:niform(?:(?:_dens)?)|ser)|val)", NAME_BUILTIN),
        Rule::bygroups(r"(?m)([a-z]+)(:)", vec![Some(TokenType::new(&["Literal", "String", "Atom"])), Some(PUNCTUATION)]),
