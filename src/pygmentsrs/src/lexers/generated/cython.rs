...
fn build_table() -> Table {
    ...
    m.insert(r"cdef", vec![
        Rule::token(r"(?m)(public|readonly|extern|api|inline|packed)\b", KEYWORD_RESERVED),
        Rule::bygroups_to(r"(?m)(struct|enum|union|class|cppclass)\b(\s+)([a-zA-Z_]\w*)", vec![Some(KEYWORD), Some(WHITESPACE), Some(NAME_CLASS)], NewState::Pop(1)),
        Rule::bygroups_to(r"(?m)([a-zA-Z_]\w*)(\s*)(?=\()", vec![Some(NAME_FUNCTION), Some(WHITESPACE)], NewState::Pop(1)),
        Rule::bygroups_to(r"(?m)([a-zA-Z_]\w*)(\s*)(?=[:,=#\n]|$)", vec![Some(NAME_VARIABLE), Some(WHITESPACE)], NewState::Pop(1)),
        Rule::bygroups(r"(?m)([a-zA-Z_]\w*)(\s*)(,)", vec![Some(NAME_VARIABLE), Some(WHITESPACE), Some(PUNCTUATION)]),
    ...
    m.insert(r"fromimport", vec![
        Rule::bygroups_to(r"(?m)(\s+)(c?import)\b", vec![Some(WHITESPACE), Some(KEYWORD)], NewState::Pop(1)),
        Rule::token(r"(?m)[a-zA-Z_.][\w.]*", NAME_NAMESPACE),
        Rule::default(NewState::Pop(1)),
    ]);
    Table(m)
