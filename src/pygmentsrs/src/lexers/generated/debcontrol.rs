...
fn build_table() -> Table {
    ...
    m.insert(
        ...
        vec![
            Rule::token_to(
                r"(?m)^(Description)",
                KEYWORD,
                NewState::Push(vec![r"description"]),
            ),
            Rule::bygroups_to(
                r"(?m)^(Maintainer|Uploaders|Changed-By)(:)(\s*)",
                vec![Some(KEYWORD), Some(PUNCTUATION), Some(WHITESPACE)],
                NewState::Push(vec![r"maintainer"]),
            ),
            Rule::bygroups_to(
                r"(?m)^((?:Build-|Pre-)?Depends(?:-Indep|-Arch)?)(:)(\s*)",
                vec![Some(KEYWORD), Some(PUNCTUATION), Some(WHITESPACE)],
                NewState::Push(vec![r"package_list"]),
            ),
            Rule::bygroups_to(
                r"(?m)^(Recommends|Suggests|Enhances|Breaks|Replaces|Provides|Conflicts)(:)(\s*)",
                vec![Some(KEYWORD), Some(PUNCTUATION), Some(WHITESPACE)],
                NewState::Push(vec![r"package_list"]),
            ),
            Rule::bygroups(
                r"(?m)^((?:Python-)?Version)(:)(\s*)(\S+)$",
                vec![
                    Some(KEYWORD),
                    Some(PUNCTUATION),
                    Some(WHITESPACE),
    ...
    m.insert(
        ...
        vec![
            Rule::bygroups(
                r"(?m)(.*)(Homepage)(: )(\S+)",
                vec![Some(TEXT), Some(STRING), Some(NAME), Some(NAME_CLASS)],
            ),
            Rule::token(r"(?m):.*\n", GENERIC_STRONG),
            Rule::token(r"(?m) .*\n", TEXT),
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)\(",
                PUNCTUATION,
                NewState::Push(vec![r"package_list_vers"]),
            ),
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::bygroups(
                r"(?m)([><=]+)(\s*)([^)]+)",
                vec![Some(OPERATOR), Some(WHITESPACE), Some(NUMBER)],
            ),
        ],
    );
    Table(m)
