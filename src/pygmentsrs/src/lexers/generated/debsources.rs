...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        ...
        Rule::bygroups_to(r"(?m)^(deb(?:-src)?)(\s+)", vec![Some(KEYWORD), Some(WHITESPACE)], NewState::Push(vec![r"distribution"])),
    ]);
    m.insert(r"distribution", vec![
        Rule::token_to(r"(?m)#.*?$", COMMENT, NewState::Pop(1)),
        Rule::token(r"(?m)\$\(ARCH\)", NAME_VARIABLE),
        Rule::token(r"(?m)[^\s$\[]+", STRING),
        Rule::token_to(r"(?m)\[", STRING_OTHER, NewState::Push(vec![r"escaped-distribution"])),
        Rule::token(r"(?m)\$", STRING),
        Rule::token_to(r"(?m)\s+", WHITESPACE, NewState::Push(vec![r"components"])),
    ]);
    m.insert(r"escaped-distribution", vec![
        Rule::token_to(r"(?m)\]", STRING_OTHER, NewState::Pop(1)),
        Rule::token(r"(?m)\$\(ARCH\)", NAME_VARIABLE),
        Rule::token(r"(?m)[^\]$]+", STRING_OTHER),
        Rule::token(r"(?m)\$", STRING_OTHER),
    ]);
    m.insert(r"components", vec![
