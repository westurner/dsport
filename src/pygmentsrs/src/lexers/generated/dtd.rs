...
fn build_table() -> Table {
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::bygroups_to(
                r"(?ms)(<!ELEMENT)(\s+)(\S+)",
                vec![Some(KEYWORD), Some(TEXT), Some(NAME_TAG)],
                NewState::Push(vec![r"element"]),
            ),
            Rule::bygroups_to(
                r"(?ms)(<!ATTLIST)(\s+)(\S+)",
                vec![Some(KEYWORD), Some(TEXT), Some(NAME_TAG)],
                NewState::Push(vec![r"attlist"]),
            ),
            Rule::bygroups_to(
                r"(?ms)(<!ENTITY)(\s+)(\S+)",
                vec![Some(KEYWORD), Some(TEXT), Some(NAME_ENTITY)],
                NewState::Push(vec![r"entity"]),
            ),
            Rule::bygroups_to(
                r"(?ms)(<!NOTATION)(\s+)(\S+)",
                vec![Some(KEYWORD), Some(TEXT), Some(NAME_TAG)],
                NewState::Push(vec![r"notation"]),
            ),
            Rule::bygroups(
                r"(?ms)(<!\[)([^\[\s]+)(\s*)(\[)",
                vec![Some(KEYWORD), Some(NAME_ENTITY), Some(TEXT), Some(KEYWORD)],
            ),
            Rule::bygroups(
                r"(?ms)(<!DOCTYPE)(\s+)([^>\s]+)",
                vec![Some(KEYWORD), Some(TEXT), Some(NAME_TAG)],
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token(r"(?ms)\'[^\']*\'", STRING_SINGLE),
            Rule::token(
                r"(?ms)CDATA|IDREFS|IDREF|ID|NMTOKENS|NMTOKEN|ENTITIES|ENTITY|NOTATION",
                KEYWORD_CONSTANT,
            ),
