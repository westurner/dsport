...
fn build_table() -> Table {
    ...
    m.insert(r"inheritance", vec![
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token(r"(?m):|,", PUNCTUATION),
        Rule::bygroups(r"(?m)(?:(\w+)(::))?(\w+)", vec![Some(NAME_NAMESPACE), Some(PUNCTUATION), Some(NAME_CLASS)]),
        Rule::token_to(r"(?m)\{", PUNCTUATION, NewState::Pop(1)),
    ]);
    m.insert(r"using", vec![
        Rule::token(r"(?m)[ \t]+", WHITESPACE),
        Rule::bygroups(r"(?m)(\[)(\w+)(\])", vec![Some(PUNCTUATION), Some(COMMENT_SPECIAL), Some(PUNCTUATION)]),
        Rule::bygroups(r#"(?m)(\")?([\w.]+)(\")?"#, vec![Some(PUNCTUATION), Some(NAME_NAMESPACE), Some(PUNCTUATION)]),
        Rule::token_to(r"(?m)::", PUNCTUATION, NewState::Push(vec![r"usingClass"])),
        Rule::default(NewState::Pop(1)),
    ]);
    m.insert(r"usingClass", vec![
        Rule::token(r"(?m)[ \t]+", WHITESPACE),
        Rule::bygroups_to(r"(?m)(as)(\s+)(\w+)", vec![Some(KEYWORD_DECLARATION), Some(WHITESPACE), Some(NAME_CLASS)], NewState::Pop(2)),
        Rule::token(r"(?m)[\w$]+", NAME_CLASS),
        Rule::default(NewState::Pop(2)),
    ]);
    m.insert(r"facet", vec![
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token_to(r"(?m)\{", PUNCTUATION, NewState::Push(vec![r"facetFields"])),
        Rule::default(NewState::Pop(1)),
    ]);
    m.insert(r"facetFields", vec![
        Rule::token(r"(?m)(?s)/\*.*?\*/", COMMENT_MULTILINE),
        Rule::token(r"(?m)//.*?$", COMMENT_SINGLE),
        Rule::token(r"(?m)\*\*.*?$", COMMENT_SPECIAL),
        Rule::token(r"(?m)#.*$", COMMENT_SINGLE),
        ...
        Rule::token(r"(?m)\+\+|\-\-|\+|\-|\*|/|\|\||&&|<=>|<=|<|>=|>|=|!|\[|\]", OPERATOR),
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::bygroups(r"(?m)(\s*)(\w+)(\s*)(=)", vec![Some(WHITESPACE), Some(NAME), Some(WHITESPACE), Some(OPERATOR)]),
        Rule::token_to(r"(?m)\}", PUNCTUATION, NewState::Pop(1)),
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token(r"(?m).", TEXT),
    ]);
