...
fn build_table() -> Table {
    ...
    m.insert(r"examples_table", vec![
        Rule::token_to(r"(?m)\s+\|", KEYWORD, NewState::Push(vec![r"examples_table_header"])),
        Rule::token(r"(?m)^\s*#.*$", COMMENT),
        Rule::token(r"(?m)(\s|.)", NAME_FUNCTION),
    ]);
    m.insert(r"examples_table_header", vec![
    ...
    m.insert(r"py_string", vec![
        Rule::token_to(r#"(?m)""""#, KEYWORD, NewState::Pop(1)),
        Rule::token(r"(?m)(<[^>]+>)", NAME_VARIABLE),
        Rule::token(r"(?m)(\s|.)", STRING),
    ]);
    m.insert(r"step_content_root", vec![
        Rule::token_to(r"(?m)$", KEYWORD, NewState::Pop(1)),
    ...
    m.insert(r"step_content_stack", vec![
