...
fn build_table() -> Table {
    ...
    m.insert(r"value", vec![
        ...
        Rule::token_to(r"(?m)-?\d+(\.\d+)?([eE][+-]?\d+)?", NUMBER_FLOAT, NewState::Pop(1)),
        ...
        Rule::token_to(r"(?m)\[", PUNCTUATION, NewState::Push(vec![r"#pop", r"list_value"])),
        Rule::token_to(r"(?m)\{", PUNCTUATION, NewState::Push(vec![r"#pop", r"object_value"])),
    ]);
    m.insert(r"list_value", vec![
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token(r"(?m)#.*$", COMMENT),
        Rule::token(r"(?m),", PUNCTUATION),
    ...
    m.insert(r"operation", vec![
        ...
        Rule::token_to(r"(?m)\(", PUNCTUATION, NewState::Push(vec![r"variable_definition"])),
        Rule::token_to(r"(?m)\{", PUNCTUATION, NewState::Push(vec![r"#pop", r"selection_set"])),
    ]);
    m.insert(r"variable_definition", vec![
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token(r"(?m)#.*$", COMMENT),
        Rule::token(r"(?m),", PUNCTUATION),
    ...
    m.insert(r"type", vec![
        ...
        Rule::token(r"(?m),", PUNCTUATION),
        Rule::token(r"(?m)\[", PUNCTUATION),
        Rule::token_to(r"(?m)(Boolean|Float|I(?:D|nt)|String)\b", NAME_BUILTIN, NewState::Pop(1)),
        Rule::token_to(r"(?m)[a-zA-Z_]\w*", NAME_CLASS, NewState::Pop(1)),
    ]);
    m.insert(r"selection_set", vec![
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token(r"(?m)#.*$", COMMENT),
        Rule::token(r"(?m),", PUNCTUATION),
        Rule::bygroups(r"(?m)([a-zA-Z_]\w*)(\s*)(:)", vec![Some(NAME_LABEL), Some(WHITESPACE), Some(PUNCTUATION)]),
        Rule::token(r"(?m)[a-zA-Z_]\w*", NAME),
        Rule::bygroups_to(r"(?m)(\.\.\.)(\s+)(on)\b", vec![Some(PUNCTUATION), Some(WHITESPACE), Some(KEYWORD)], NewState::Push(vec![r"inline_fragment"])),
        Rule::token_to(r"(?m)\.\.\.", PUNCTUATION, NewState::Push(vec![r"fragment_spread"])),
        Rule::token_to(r"(?m)\(", PUNCTUATION, NewState::Push(vec![r"arguments"])),
        Rule::token_to(r"(?m)@[a-zA-Z_]\w*", NAME_DECORATOR, NewState::Push(vec![r"directive"])),
        Rule::token_to(r"(?m)\{", PUNCTUATION, NewState::Push(vec![r"selection_set"])),
        Rule::token_to(r"(?m)\}", PUNCTUATION, NewState::Pop(1)),
    ]);
    m.insert(r"directive", vec![
        ...
        Rule::token(r"(?m)#.*$", COMMENT),
        Rule::token(r"(?m),", PUNCTUATION),
        Rule::token_to(r"(?m)\(", PUNCTUATION, NewState::Push(vec![r"#pop", r"arguments"])),
    ]);
    ...
    m.insert(r"fragment_definition", vec![
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token(r"(?m)#.*$", COMMENT),
        Rule::token(r"(?m),", PUNCTUATION),
        Rule::token(r"(?m)[\]!]", PUNCTUATION),
    ...
    m.insert(r"fragment_spread", vec![
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token(r"(?m)#.*$", COMMENT),
        Rule::token(r"(?m),", PUNCTUATION),
        Rule::token_to(r"(?m)@[a-zA-Z_]\w*", NAME_DECORATOR, NewState::Push(vec![r"directive"])),
    ...
    m.insert(r"inline_fragment", vec![
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token(r"(?m)#.*$", COMMENT),
        Rule::token(r"(?m),", PUNCTUATION),
