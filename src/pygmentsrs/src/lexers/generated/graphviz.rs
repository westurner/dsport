...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        ...
        Rule::token(r"(?m)/(\\\n)?[*](.|\n)*?[*](\\\n)?/", COMMENT_MULTILINE),
        Rule::token(r"(?m)(?i)(node|edge|graph|digraph|subgraph|strict)\b", KEYWORD),
        Rule::token(r"(?m)--|->", OPERATOR),
        Rule::token(r"(?m)[{}\[\]:;,]", PUNCTUATION),
        Rule::bygroups_to(r"(?m)(\b\D\w*)(\s*)(=)(\s*)", vec![Some(NAME_ATTRIBUTE), Some(WHITESPACE), Some(PUNCTUATION), Some(WHITESPACE)], NewState::Push(vec![r"attr_id"])),
    ...
    m.insert(r"attr_id", vec![
        Rule::token_to(r"(?m)\b\D\w*", STRING, NewState::Pop(1)),
        Rule::token_to(r"(?m)[-]?((\.[0-9]+)|([0-9]+(\.[0-9]*)?))", NUMBER, NewState::Pop(1)),
