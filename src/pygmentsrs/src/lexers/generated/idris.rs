//! AUTO-GENERATED from `pygments.pygments.lexers.haskell:IdrisLexer`.
//!
//! Do not edit by hand. Regenerate with:
//!   python tools/gen_lexer.py pygments.lexers.haskell:IdrisLexer:idris
...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        Rule::bygroups(r"(?m)^(\s*)(%(lib|link|flag|include|hide|freeze|access|default|logging|dynamic|name|error_handlers|language))", vec![Some(WHITESPACE), Some(KEYWORD_RESERVED)]),
        Rule::bygroups(r"(?m)(\s*)(--(?![!#$%&*+./<=>?@^|_~:\\]).*?)$", vec![Some(WHITESPACE), Some(COMMENT_SINGLE)]),
        Rule::bygroups(r"(?m)(\s*)(\|{3}.*?)$", vec![Some(WHITESPACE), Some(COMMENT_SINGLE)]),
        Rule::bygroups_to(r"(?m)(\s*)(\{-)", vec![Some(WHITESPACE), Some(COMMENT_MULTILINE)], NewState::Push(vec![r"comment"])),
        Rule::bygroups(r"(?m)^(\s*)([^\s(){}]+)(\s*)(:)(\s*)", vec![Some(WHITESPACE), Some(NAME_FUNCTION), Some(WHITESPACE), Some(OPERATOR_WORD), Some(WHITESPACE)]),
        Rule::token(r"(?m)\b(case|class|data|default|using|do|else|if|in|infix[lr]?|instance|rewrite|auto|namespace|codata|mutual|private|public|abstract|total|partial|interface|implementation|export|covering|constructor|let|proof|of|then|static|where|_|with|pattern|term|syntax|prefix|postulate|parameters|record|dsl|impossible|implicit|tactics|intros|intro|compute|refine|exact|trivial)(?!\')\b", KEYWORD_RESERVED),
