...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        Rule::token_to(r"(?m)/\*\*(?!/)", COMMENT_MULTILINE, NewState::Push(vec![r"documentation comment"])),
        ...
        Rule::token_to(r"(?m)grammar\b", KEYWORD_RESERVED, NewState::Push(vec![r"grammar name"])),
        Rule::bygroups(r"(?m)(<)(NULL|VOID)(>)", vec![Some(PUNCTUATION), Some(NAME_BUILTIN), Some(PUNCTUATION)]),
    ...
    m.insert(r"grammar name", vec![
        Rule::token_to(r"(?m);", PUNCTUATION, NewState::Pop(1)),
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token(r"(?m)\.", PUNCTUATION),
        Rule::token(r"(?m)[^;\s.]+", NAME_NAMESPACE),
    ]);
    m.insert(r"rulename", vec![
        Rule::token_to(r"(?m)>", PUNCTUATION, NewState::Pop(1)),
        Rule::token(r"(?m)\*", PUNCTUATION),
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::bygroups(r"(?m)([^.>]+)(\s*)(\.)", vec![Some(NAME_NAMESPACE), Some(TEXT), Some(PUNCTUATION)]),
        Rule::token(r"(?m)[^.>]+", NAME_CONSTANT),
    ]);
    m.insert(r"documentation comment", vec![
        Rule::token_to(r"(?m)\*/", COMMENT_MULTILINE, NewState::Pop(1)),
        Rule::bygroups_g(r"(?m)^(\s*)(\*?)(\s*)(@(?:example|see))(\s+)([\w\W]*?(?=(?:^\s*\*?\s*@|\*/)))", vec![Some(GroupAction::Token(WHITESPACE)), Some(GroupAction::Token(COMMENT_MULTILINE)), Some(GroupAction::Token(WHITESPACE)), Some(GroupAction::Token(COMMENT_SPECIAL)), Some(GroupAction::Token(WHITESPACE)), Some(GroupAction::UsingThis { state: Some(vec!["root", "example"]) })]),
        Rule::bygroups(r"(?m)(^\s*\*?\s*)(@\S*)", vec![Some(COMMENT_MULTILINE), Some(COMMENT_SPECIAL)]),
        Rule::token(r"(?m)[^*\n@]+|\w|\W", COMMENT_MULTILINE),
    ]);
    m.insert(r"example", vec![
        Rule::bygroups(r"(?m)(\n\s*)(\*)", vec![Some(WHITESPACE), Some(COMMENT_MULTILINE)]),
        Rule::token(r"(?m)\A#JSGF[^;]*", COMMENT_PREPROC),
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token(r"(?m);", PUNCTUATION),
        Rule::token(r"(?m)[=|()\[\]*+]", OPERATOR),
