...
fn build_table() -> Table {
    ...
    m.insert(r"local_name", vec![
        Rule::token_to(r"(?m)[^\W\d]\w*(?=\()", NAME_FUNCTION, NewState::Push(vec![r"function_params"])),
        Rule::token(r"(?m)[^\W\d]\w*", NAME_VARIABLE),
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token_to(r"(?m)(?==)", WHITESPACE, NewState::Push(vec![r"#pop", r"local_value"])),
    ]);
    ...
    m.insert(r"function_params", vec![
        Rule::token(r"(?m)[^\W\d]\w*", NAME_VARIABLE),
        Rule::token(r"(?m)\(", PUNCTUATION),
        Rule::token_to(r"(?m)\)", PUNCTUATION, NewState::Pop(1)),
        Rule::token(r"(?m),", PUNCTUATION),
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token_to(r"(?m)=", OPERATOR, NewState::Push(vec![r"function_param_default"])),
    ]);
    ...
    m.insert(r"object", vec![
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token_to(r"(?m)local\b", KEYWORD, NewState::Push(vec![r"object_local_name"])),
        Rule::token_to(r"(?m)assert\b", KEYWORD, NewState::Push(vec![r"object_assert"])),
        Rule::token_to(r"(?m)\[", OPERATOR, NewState::Push(vec![r"field_name_expr"])),
        Rule::token_to(r"(?m)(?=[^\W\d]\w*)", TEXT, NewState::Push(vec![r"field_name"])),
        Rule::token_to(r"(?m)\}", PUNCTUATION, NewState::Pop(1)),
    ...
    m.insert(r"field_name", vec![
        Rule::token_to(r"(?m)[^\W\d]\w*(?=\()", NAME_FUNCTION, NewState::Push(vec![r"field_separator", r"function_params"])),
        Rule::token_to(r"(?m)[^\W\d]\w*", NAME_VARIABLE, NewState::Push(vec![r"field_separator"])),
    ]);
    m.insert(r"double_field_name", vec![
        Rule::token_to(r#"(?m)([^"\\]|\\.)*""#, NAME_VARIABLE, NewState::Push(vec![r"field_separator"])),
    ]);
    ...
    m.insert(r"field_separator", vec![
        Rule::token(r"(?m)\s+", WHITESPACE),
        Rule::token_to(r"(?m)\+?::?:?", PUNCTUATION, NewState::Push(vec![r"#pop", r"#pop", r"field_value"])),
        Rule::token(r"(?m)(//|#).*\n", COMMENT_SINGLE),
        Rule::token(r"(?m)/\*\*([^/]|/(?!\*))*\*/", STRING_DOC),
        Rule::token(r"(?m)/\*([^/]|/(?!\*))*\*/", COMMENT),
    ]);
    ...
    m.insert(r"object_local_name", vec![
        Rule::token_to(r"(?m)[^\W\d]\w*", NAME_VARIABLE, NewState::Push(vec![r"#pop", r"object_local_value"])),
        Rule::token(r"(?m)\s+", WHITESPACE),
    ]);
