...
fn build_table() -> Table {
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::bygroups_to(
                ...
                NewState::Push(vec![r"after-control"]),
            ),
            Rule::bygroups(
                r"(?m)(deleteoldrdn)(:)([ \n]*)([0-1]+)([ \t]*\n)",
                vec![
            ...
            Rule::bygroups_to(
                r"(?m)(dn|newrdn)(::)",
                vec![Some(KEYWORD), Some(PUNCTUATION)],
                NewState::Push(vec![r"base64-dn"]),
            ),
            Rule::bygroups_to(
                r"(?m)(dn|newrdn)(:)",
                vec![Some(KEYWORD), Some(PUNCTUATION)],
                NewState::Push(vec![r"dn"]),
            ),
            Rule::bygroups(
                r"(?m)(objectclass)(:)([ \t]*)([^ \t\n]*)([ \t]*\n)",
                vec![
            ...
            Rule::bygroups_to(
                r"(?m)([a-zA-Z]*|[0-9][0-9\.]*[0-9])(;)",
                vec![Some(NAME_ATTRIBUTE), Some(PUNCTUATION)],
                NewState::Push(vec![r"property"]),
            ),
            Rule::bygroups_to(
                r"(?m)([a-zA-Z]*|[0-9][0-9\.]*[0-9])(:<)",
                vec![Some(NAME_ATTRIBUTE), Some(PUNCTUATION)],
                NewState::Push(vec![r"url"]),
            ),
            Rule::bygroups_to(
                r"(?m)([a-zA-Z]*|[0-9][0-9\.]*[0-9])(::?)",
                vec![Some(NAME_ATTRIBUTE), Some(PUNCTUATION)],
    ...
    m.insert(
        ...
        vec![
            Rule::bygroups(
                r"(?m)([-a-zA-Z0-9]*)(;)",
                vec![Some(NAME_PROPERTY), Some(PUNCTUATION)],
            ),
            Rule::bygroups_to(
                r"(?m)([-a-zA-Z0-9]*)(:<)",
                vec![Some(NAME_PROPERTY), Some(PUNCTUATION)],
    ...
    m.insert(
        ...
        vec![Rule::bygroups_to(
            r"(?m)([ \t]*)([-a-zA-Z0-9\.]+)(=)",
            vec![Some(WHITESPACE), Some(NAME_ATTRIBUTE), Some(OPERATOR)],
            NewState::Push(vec![r"#pop", r"dn-value"]),
        )],
    );
    m.insert(
        r"dn-value",
