//! AUTO-GENERATED from `pygments.pygments.lexers.scripting:LuaLexer`.
//!
//! Do not edit by hand. Regenerate with:
//!   python tools/gen_lexer.py pygments.lexers.scripting:LuaLexer:lua
...
fn build_table() -> Table {
    ...
    m.insert(r"base", vec![
        ...
        Rule::token(r"(?m)(?:--.*$)", COMMENT_SINGLE),
        Rule::token(r"(?m)(?:\s+(?!\s))", WHITESPACE),
        Rule::token(r"(?m)(?i)0x[\da-f]*(\.[\da-f]*)?(p[+-]?\d+)?", NUMBER_HEX),
        ...
        Rule::token(r"(?m)(break|do|e(?:lse(?:(?:if)?)|nd)|for|i(?:[fn])|re(?:peat|turn)|then|until|while)\b", KEYWORD_RESERVED),
        Rule::token_to(r"(?m)goto\b", KEYWORD_RESERVED, NewState::Push(vec![r"goto"])),
        Rule::token(r"(?m)(local)\b", KEYWORD_DECLARATION),
        Rule::token(r"(?m)(true|false|nil)\b", KEYWORD_CONSTANT),
        Rule::token_to(r"(?m)(function)\b", KEYWORD_RESERVED, NewState::Push(vec![r"funcname"])),
        ...
        Rule::token_to(r"(?m)[A-Za-z_]\w*(?=\s*[.:])", NAME_VARIABLE, NewState::Push(vec![r"varname"])),
        Rule::token(r"(?m)[A-Za-z_]\w*(?=\s*\()", NAME_FUNCTION),
    ...
    m.insert(r"stringescape", vec![
        Rule::token(r#"(?m)\\([abfnrtv\\"\']|[\r\n]{1,2}|z\s*|x[0-9a-fA-F]{2}|\d{1,3}|u\{[0-9a-fA-F]+\})"#, STRING_ESCAPE),
    ]);
    m.insert(r"sqs", vec![
    ...
    m.insert(r"varname", vec![
        ...
        Rule::token_to(r"(?m)(?:[^\W\d]\w*)(?=\s*\()", NAME_FUNCTION, NewState::Pop(1)),
