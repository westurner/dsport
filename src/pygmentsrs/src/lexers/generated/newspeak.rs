...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        ...
        Rule::bygroups(r"(?m)\b(class)(\s+)(\w+)(\s*)", vec![Some(KEYWORD_DECLARATION), Some(TEXT), Some(NAME_CLASS), Some(TEXT)]),
        Rule::token(r"(?m)\b(mixin|self|super|private|public|protected|nil|true|false)\b", KEYWORD),
        Rule::bygroups(r"(?m)(\w+\:)(\s*)([a-zA-Z_]\w+)", vec![Some(NAME_FUNCTION), Some(TEXT), Some(NAME_VARIABLE)]),
        Rule::bygroups(r"(?m)(\w+)(\s*)(=)", vec![Some(NAME_ATTRIBUTE), Some(TEXT), Some(OPERATOR)]),
