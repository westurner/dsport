...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        Rule::bygroups(r"(?m)(include)(\s+)([^\s;]+)", vec![Some(KEYWORD), Some(WHITESPACE), Some(NAME)]),
        Rule::token_to(r"(?m)[^\s;#]+", KEYWORD, NewState::Push(vec![r"stmt"])),
        Rule::token(r"(?m)#.*\n", COMMENT_SINGLE),
        Rule::token(r"(?m)on|off", NAME_CONSTANT),
        Rule::token(r"(?m)\$[^\s;#()]+", NAME_VARIABLE),
        Rule::bygroups(r"(?m)([a-z0-9.-]+)(:)([0-9]+)", vec![Some(NAME), Some(PUNCTUATION), Some(NUMBER_INTEGER)]),
        Rule::token(r"(?m)[a-z-]+/[a-z-+]+", STRING),
        Rule::token(r"(?m)[0-9]+[km]?\b", NUMBER_INTEGER),
        Rule::bygroups(r"(?m)(~)(\s*)([^\s{]+)", vec![Some(PUNCTUATION), Some(WHITESPACE), Some(STRING_REGEX)]),
    ...
    m.insert(r"block", vec![
        ...
        Rule::token_to(r"(?m)[^\s;#]+", KEYWORD_NAMESPACE, NewState::Push(vec![r"stmt"])),
        Rule::token(r"(?m)#.*\n", COMMENT_SINGLE),
        Rule::token(r"(?m)on|off", NAME_CONSTANT),
        Rule::token(r"(?m)\$[^\s;#()]+", NAME_VARIABLE),
        Rule::bygroups(r"(?m)([a-z0-9.-]+)(:)([0-9]+)", vec![Some(NAME), Some(PUNCTUATION), Some(NUMBER_INTEGER)]),
