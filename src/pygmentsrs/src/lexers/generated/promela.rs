...
//! AUTO-GENERATED from `pygments.pygments.lexers.c_like:PromelaLexer`.
//!
//! Do not edit by hand. Regenerate with:
//!   python tools/gen_lexer.py pygments.lexers.c_like:PromelaLexer:promela
...
fn build_table() -> Table {
    ...
    m.insert(r"statements", vec![
        Rule::token(r"(?m)(\[\]|<>|/\\|\\/)|(U|W|V)\b", OPERATOR),
        Rule::token(r"(?m)@", PUNCTUATION),
        Rule::bygroups(r"(?m)(\.)([a-zA-Z_]\w*)", vec![Some(OPERATOR), Some(NAME_ATTRIBUTE)]),
        Rule::token(r"(?m)(atomic|break|d(?:_step|o)|f(?:i|or)|goto|i(?:[fn])|od|unless)\b", KEYWORD),
        Rule::token(r"(?m)(assert|get_priority|print(?:[fm])|set_priority)\b", NAME_FUNCTION),
        Rule::token(r"(?m)(c_(?:code|decl|expr|state|track))\b", KEYWORD),
        Rule::token(r"(?m)(STDIN|_(?:(?:last|nr_pr|p(?:id|riority))?)|else|np_)\b", NAME_BUILTIN),
        Rule::token(r"(?m)(e(?:mpty|nabled|val)|full|len|n(?:empty|full)|pc_value)\b", NAME_FUNCTION),
        Rule::token(r"(?m)run\b", OPERATOR_WORD),
        Rule::token(r"(?m)(D_proctype|active|chan|hidden|init|local|mtype|n(?:ever|otrace)|proctype|show|t(?:race|ypedef)|x(?:[rs]))\b", KEYWORD_DECLARATION),
        Rule::token(r"(?m)(pr(?:iority|ovided))\b", KEYWORD),
        Rule::token(r"(?m)(inline|ltl|select)\b", KEYWORD_DECLARATION),
        Rule::token(r"(?m)skip\b", KEYWORD),
        Rule::token(r"(?m)(b(?:it|ool|yte)|int|pid|short|unsigned)\b", KEYWORD_TYPE),
    ...
    m.insert(
        r"types",
        vec![Rule::token(
            r"(?m)(b(?:it|ool|yte)|int|pid|short|unsigned)\b",
            KEYWORD_TYPE,
        )],
    );
