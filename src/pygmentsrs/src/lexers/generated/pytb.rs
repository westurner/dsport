...
fn build_table() -> Table {
    ...
    m.insert(
        ...
        vec![
            Rule::token(r"(?m)\n", WHITESPACE),
            Rule::token_to(
                r"(?m)^(\^C)?Traceback \(most recent call last\):\n",
                GENERIC_TRACEBACK,
            ...
            Rule::token(
                r"(?m)^The above exception was the direct cause of the following exception:\n\n",
                GENERIC_TRACEBACK,
            ),
            Rule::token_to(
                r#"(?m)^(?=  File "[^"]+", line \d+)"#,
    ...
    m.insert(
        ...
        vec![
            Rule::bygroups_to(
                r"(?m)^( {4,})([~^]+)(\n)",
                vec![Some(WHITESPACE), Some(PUNCTUATION_MARKER), Some(WHITESPACE)],
                NewState::Pop(1),
            ),
            Rule::default(NewState::Pop(1)),
        ],
