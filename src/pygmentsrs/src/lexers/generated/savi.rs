...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        ...
        Rule::token_to(r#"(?m)\w?""#, STRING_DOUBLE, NewState::Push(vec![r"string.double"])),
        Rule::token_to(r"(?m)'", STRING_CHAR, NewState::Push(vec![r"string.char"])),
        Rule::token(r"(?m)(_?[A-Z]\w*)", NAME_CLASS),
        Rule::bygroups(r"(?m)(\.)(\s*)(_?[A-Z]\w*)", vec![Some(PUNCTUATION), Some(WHITESPACE), Some(NAME_CLASS)]),
        Rule::bygroups_to(r"(?m)^([ \t]*)(:\w+)", vec![Some(WHITESPACE), Some(NAME_TAG)], NewState::Push(vec![r"decl"])),
        ...
        Rule::bygroups(r"(?m)(\.)(\s*)(\w+)", vec![Some(PUNCTUATION), Some(WHITESPACE), Some(NAME_FUNCTION)]),
        Rule::bygroups(r"(?m)(@)(\w+)", vec![Some(PUNCTUATION), Some(NAME_FUNCTION)]),
        Rule::token_to(r"(?m)\(", PUNCTUATION, NewState::Push(vec![r"root"])),
        Rule::token_to(r"(?m)\)", PUNCTUATION, NewState::Pop(1)),
        Rule::token_to(r"(?m)\{", PUNCTUATION, NewState::Push(vec![r"root"])),
        Rule::token_to(r"(?m)\}", PUNCTUATION, NewState::Pop(1)),
        Rule::token_to(r"(?m)\[", PUNCTUATION, NewState::Push(vec![r"root"])),
        Rule::bygroups_to(r"(?m)(\])(\!)", vec![Some(PUNCTUATION), Some(GENERIC_DELETED)], NewState::Pop(1)),
        Rule::token_to(r"(?m)\]", PUNCTUATION, NewState::Pop(1)),
    ...
    m.insert(r"string.double", vec![
        Rule::token_to(r"(?m)\\\(", STRING_INTERPOL, NewState::Push(vec![r"string.interpolation"])),
    ...
    m.insert(r"string.interpolation", vec![
