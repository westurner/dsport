...
fn build_table() -> Table {
    ...
    m.insert(r"operators", vec![Rule::token(r"(?m)[,:;]", PUNCTUATION)]);
    m.insert(
        r"literals",
        vec![
            ...
            Rule::token_to(
                r#"(?m)""#,
                STRING_DOUBLE,
                NewState::Push(vec![r"literals.string_double"]),
            ),
            Rule::token_to(
                r"(?m)'",
                STRING_SINGLE,
                NewState::Push(vec![r"literals.string_single"]),
            ),
        ],
    );
    m.insert(
        r"literals.string_double",
        vec![
