...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        ...
        Rule::token_to(r"(?m)^1\.\.\d+", KEYWORD_DECLARATION, NewState::Push(vec![r"plan"])),
        Rule::bygroups_to(r"(?m)^(not ok)([^\S\n]*)(\d*)", vec![Some(GENERIC_ERROR), Some(TEXT), Some(NUMBER_INTEGER)], NewState::Push(vec![r"test"])),
        Rule::bygroups_to(r"(?m)^(ok)([^\S\n]*)(\d*)", vec![Some(KEYWORD_RESERVED), Some(TEXT), Some(NUMBER_INTEGER)], NewState::Push(vec![r"test"])),
        Rule::token(r"(?m)^#.*\n", COMMENT),
