//! AUTO-GENERATED from `pygments.pygments.lexers.asm:TasmLexer`.
//!
//! Do not edit by hand. Regenerate with:
//!   python tools/gen_lexer.py pygments.lexers.asm:TasmLexer:tasm
...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        ...
        Rule::token_to(r"(?im)BITS|USE16|USE32|SECTION|SEGMENT|ABSOLUTE|EXTERN|GLOBAL|ORG|ALIGN|STRUC|ENDSTRUC|ENDS|COMMON|CPU|GROUP|UPPERCASE|INCLUDE|EXPORT|LIBRARY|MODULE|PROC|ENDP|USES|ARG|DATASEG|UDATASEG|END|IDEAL|P386|MODEL|ASSUME|CODESEG|SIZE", KEYWORD, NewState::Push(vec![r"instruction-args"])),
        Rule::bygroups_to(r"(?im)([@a-z$._?][\w$.?#@~]*)(\s+)(db|dd|dw|T[A-Z][a-z]+)", vec![Some(NAME_CONSTANT), Some(WHITESPACE), Some(KEYWORD_DECLARATION)], NewState::Push(vec![r"instruction-args"])),
    ...
    m.insert(r"instruction-args", vec![
        ...
        Rule::token(r"(?im)seg|wrt|strict", OPERATOR_WORD),
        Rule::token(r"(?im)byte|[dq]?word", KEYWORD_TYPE),
        Rule::token(r"(?im)(r[0-9][0-5]?[bwd]|[a-d][lh]|[er]?[a-d]x|[er]?[sb]p|[er]?[sd]i|[c-gs]s|st[0-7]|mm[0-7]|cr[0-4]|dr[0-367]|tr[3-7])\b", NAME_BUILTIN),
        Rule::token(r"(?im)[@a-z$._?][\w$.?#@~]*", NAME_VARIABLE),
        Rule::bygroups(r"(?im)(\\)(\s*)(;.*)([\r\n])", vec![Some(TEXT), Some(WHITESPACE), Some(COMMENT_SINGLE), Some(WHITESPACE)]),
