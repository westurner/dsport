...
fn build_table() -> Table {
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)/\*",
                COMMENT_MULTILINE,
                NewState::Push(vec![r"in-comment"]),
            ),
        ],
    );
    m.insert(
        ...
        vec![Rule::bygroups(
            r"(?m)(?i)^(\s*)(:[a-z0-9_]+)",
            vec![Some(WHITESPACE), Some(NAME_LABEL)],
        )],
    );
    ...
    m.insert(
        r"user-variables",
        vec![Rule::token(r"(?m)(?i)[a-z_][a-z0-9_]*", NAME_VARIABLE)],
    );
    m.insert(
        r"operators",
        vec![
    ...
    m.insert(
        ...
        vec![
            Rule::bygroups(
                r"(?m)(-?)([0-9]+)",
                vec![Some(OPERATOR), Some(NUMBER_INTEGER)],
            ),
            Rule::token(r"(?m)(?i)\$[0-9a-f]+", NUMBER_HEX),
        ],
    );
    ...
    m.insert(r"all-whitespace", vec![Rule::token(r"(?m)\s+", WHITESPACE)]);
    m.insert(
        r"in-comment",
        vec![
