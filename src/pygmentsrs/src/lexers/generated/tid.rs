...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        ...
        Rule::bygroups(r"(?m)^(```\n)([\w\W]*?)(^```$)", vec![Some(STRING), Some(TEXT), Some(STRING)]),
        Rule::dispatch_code_block(r"(?m)^(```)(\w+)(\n)([\w\W]*?)(^```$)", DispatchCodeBlockSpec {
            prefix: vec![GroupEmit { group: 1, token: STRING, skip_if_none: false }, GroupEmit { group: 2, token: STRING, skip_if_none: false }, GroupEmit { group: 3, token: TEXT, skip_if_none: false }],
            lang_group: 2,
            code_groups: vec![4],
            suffix: vec![GroupEmit { group: 5, token: STRING, skip_if_none: false }],
            fallback_token: STRING,
            strip_indent_from_group: None,
        }),
        Rule::using_lexer(r"(?m)^(<style>)(\n)([\w\W]*?)(^</style>$)", "css", None),
        Rule::token(r"(?m)^(\\(?:define|end)|c(?:aption|reated)|modified|t(?:ags|(?:itl|yp)e))\b", KEYWORD),
        Rule::token(r"(?m)\\.", TEXT),
        ...
        Rule::bygroups(r"(?m)(\s)(,,[^,]+,,)", vec![Some(TEXT), Some(GENERIC_EMPH)]),
        Rule::bygroups(r"(?m)(\s)(__[^_]+__)", vec![Some(TEXT), Some(GENERIC_STRONG)]),
        ...
        Rule::bygroups(r"(?m)(\[{2})([^\]\|]+)(\]{2})", vec![Some(TEXT), Some(NAME_TAG), Some(TEXT)]),
        Rule::bygroups(r"(?m)(\[{2})([^\]\|]+)(\|)([^\]\|]+)(\]{2})", vec![Some(TEXT), Some(NAME_TAG), Some(TEXT), Some(NAME_ATTRIBUTE), Some(TEXT)]),
        Rule::bygroups(r"(?m)(\{{2})([^}]+)(\}{2})", vec![Some(TEXT), Some(NAME_TAG), Some(TEXT)]),
