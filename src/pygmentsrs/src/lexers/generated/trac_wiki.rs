...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        ...
        Rule::bygroups_g(r"(?im)^(=+)([^=]+)(=+)(\s*#.+)?$", vec![Some(GroupAction::Token(GENERIC_HEADING)), Some(GroupAction::UsingThis { state: None }), Some(GroupAction::Token(GENERIC_HEADING)), Some(GroupAction::Token(STRING))]),
        Rule::bygroups_to(r"(?im)(\{\{\{)(\n#!.+)?", vec![Some(NAME_BUILTIN), Some(NAME_NAMESPACE)], NewState::Push(vec![r"codeblock"])),
        Rule::token(r"(?im)(\'\'\'?|\|\||`|__|~~|\^|,,|::)", COMMENT),
        Rule::bygroups(r"(?im)^( +)([.*-])( )", vec![Some(TEXT), Some(NAME_BUILTIN), Some(TEXT)]),
        Rule::bygroups(r"(?im)^( +)([a-z]{1,5}\.)( )", vec![Some(TEXT), Some(NAME_BUILTIN), Some(TEXT)]),
        Rule::token(r"(?im)\[\[\w+.*?\]\]", KEYWORD),
        Rule::bygroups(r"(?im)(\[[^\s\]]+)(\s+[^\]]+?)?(\])", vec![Some(KEYWORD), Some(STRING), Some(KEYWORD)]),
