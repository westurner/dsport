...
fn build_table() -> Table {
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)(\#(?:let|s(?:et|how)))\b",
                KEYWORD_DECLARATION,
                NewState::Push(vec![r"inline_code"]),
            ),
            Rule::token_to(
                r"(?m)(\#i(?:mport|nclude))\b",
                KEYWORD_NAMESPACE,
                NewState::Push(vec![r"inline_code"]),
            ),
            Rule::token_to(
                r"(?m)(\#(?:export|for|if|while))\b",
                KEYWORD_RESERVED,
                NewState::Push(vec![r"inline_code"]),
            ),
            ...
            Rule::bygroups_to(
                ...
                NewState::Push(vec![r"markup"]),
            ),
            Rule::bygroups_to(
                r"(?m)(#[a-zA-Z_][a-zA-Z0-9_-]*)(\()",
                vec![Some(NAME_FUNCTION), Some(PUNCTUATION)],
                NewState::Push(vec![r"code"]),
            ),
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token(
                r"(?m)(as|break|continue|e(?:lse|xport)|for|i(?:[fn])|return|while)\b",
                KEYWORD_RESERVED,
            ),
            Rule::token(r"(?m)(i(?:mport|nclude))\b", KEYWORD_NAMESPACE),
            Rule::token(r"(?m)(auto|(?:fals|non|tru)e)\b", KEYWORD_CONSTANT),
            Rule::bygroups(
                r"(?m)([0-9.]+)(mm|pt|cm|in|em|fr|%)",
                vec![Some(NUMBER), Some(KEYWORD_RESERVED)],
            ),
            Rule::token(r"(?m)0x[0-9a-fA-F]+", NUMBER_HEX),
            Rule::token(r"(?m)0b[01]+", NUMBER_BIN),
            Rule::token(r"(?m)0o[0-7]+", NUMBER_OCT),
