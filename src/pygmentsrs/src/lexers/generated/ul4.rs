...
fn build_table() -> Table {
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::bygroups_to(
                r"(?ms)(<\?)(\s*)(ul4)(\s*)([a-zA-Z_][a-zA-Z_0-9]*)?",
                vec![
                    Some(COMMENT_PREPROC),
                    Some(WHITESPACE),
                    Some(KEYWORD),
                ...
                NewState::Push(vec![r"ul4"]),
            ),
            Rule::token_to(
                r"(?ms)<\?\s*note\s*\?>",
                COMMENT,
                NewState::Push(vec![r"note"]),
            ),
            Rule::token(r"(?ms)<\?\s*note\s.*?\?>", COMMENT),
            Rule::token_to(
                r"(?ms)<\?\s*doc\s*\?>",
                STRING_DOC,
                NewState::Push(vec![r"doc"]),
            ),
            Rule::token(r"(?ms)<\?\s*doc\s.*?\?>", STRING_DOC),
            Rule::token_to(
                r"(?ms)<\?\s*ignore\s*\?>",
                COMMENT,
                NewState::Push(vec![r"ignore"]),
            ),
            Rule::bygroups_to(
                r"(?ms)(<\?)(\s*)(def)(\s*)([a-zA-Z_][a-zA-Z_0-9]*)?",
                vec![
                    Some(COMMENT_PREPROC),
                    Some(WHITESPACE),
                    Some(KEYWORD),
            ...
            Rule::bygroups_to(
                r"(?ms)(<\?)(\s*)(printx|print|for|if|elif|else|while|code|renderblocks?|render)\b",
                vec![Some(COMMENT_PREPROC), Some(WHITESPACE), Some(KEYWORD)],
                NewState::Push(vec![r"ul4"]),
            ),
            Rule::bygroups_to(
                r"(?ms)(<\?)(\s*)(end)\b",
                vec![Some(COMMENT_PREPROC), Some(WHITESPACE), Some(KEYWORD)],
                NewState::Push(vec![r"end"]),
            ),
            Rule::bygroups_to(
                r"(?ms)(<\?)(\s*)(whitespace)\b",
                vec![Some(COMMENT_PREPROC), Some(WHITESPACE), Some(KEYWORD)],
                NewState::Push(vec![r"whitespace"]),
            ),
