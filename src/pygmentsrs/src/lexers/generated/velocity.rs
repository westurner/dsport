...
fn build_table() -> Table {
    ...
    m.insert(r"root", vec![
        ...
        Rule::bygroups(r"(?ms)(##)(.*?$)", vec![Some(COMMENT_PREPROC), Some(COMMENT)]),
        Rule::bygroups_to(r"(?ms)(#\{?)([a-zA-Z_]\w*)(\}?)(\s?\()", vec![Some(COMMENT_PREPROC), Some(NAME_FUNCTION), Some(COMMENT_PREPROC), Some(PUNCTUATION)], NewState::Push(vec![r"directiveparams"])),
        Rule::bygroups(r"(?ms)(#\{?)([a-zA-Z_]\w*)(\}|\b)", vec![Some(COMMENT_PREPROC), Some(NAME_FUNCTION), Some(COMMENT_PREPROC)]),
        Rule::token_to(r"(?ms)\$!?\{?", PUNCTUATION, NewState::Push(vec![r"variable"])),
    ]);
    m.insert(r"variable", vec![
        ...
        Rule::bygroups_to(r"(?ms)(\.)([a-zA-Z_]\w*)", vec![Some(PUNCTUATION), Some(NAME_VARIABLE)], NewState::PushSame),
        Rule::token_to(r"(?ms)\}", PUNCTUATION, NewState::Pop(1)),
        Rule::default(NewState::Pop(1)),
    ]);
    m.insert(r"directiveparams", vec![
        Rule::token(r"(?ms)(&&|\|\||==?|!=?|[-<>+*%&|^/])|\b(eq|ne|gt|lt|ge|le|not|in)\b", OPERATOR),
