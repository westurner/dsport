...
fn build_table() -> Table {
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)\[",
                PUNCTUATION,
                NewState::Push(vec![r"extended_attributes"]),
            ),
            Rule::token(r"(?m)partial(?![\w-])", KEYWORD),
            Rule::token_to(
                r"(?m)typedef(?![\w-])",
                KEYWORD,
                NewState::Push(vec![r"typedef", r"type"]),
            ),
            Rule::token_to(
                r"(?m)interface(?![\w-])",
                KEYWORD,
                NewState::Push(vec![r"interface_rest"]),
            ),
            Rule::token_to(
                r"(?m)enum(?![\w-])",
                KEYWORD,
                NewState::Push(vec![r"enum_rest"]),
            ),
            Rule::token_to(
                r"(?m)callback(?![\w-])",
                KEYWORD,
                NewState::Push(vec![r"callback_rest"]),
            ),
            Rule::token_to(
                r"(?m)dictionary(?![\w-])",
                KEYWORD,
                NewState::Push(vec![r"dictionary_rest"]),
            ),
            Rule::token_to(
                r"(?m)namespace(?![\w-])",
                KEYWORD,
                NewState::Push(vec![r"namespace_rest"]),
            ),
            Rule::token_to(
                r"(?m)_?[A-Za-z][a-zA-Z0-9_-]*",
                NAME_CLASS,
                NewState::Push(vec![r"implements_rest"]),
            ),
        ],
    );
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)=",
                PUNCTUATION,
                NewState::Push(vec![r"extended_attribute_rest"]),
            ),
            Rule::token_to(
                r"(?m)\(",
                PUNCTUATION,
                NewState::Push(vec![r"argument_list"]),
            ),
            Rule::token_to(r"(?m)\]", PUNCTUATION, NewState::Pop(1)),
        ],
    );
    m.insert(
        r"extended_attribute_rest",
        vec![
            Rule::token(r"(?m)\s+", TEXT),
            Rule::token(r"(?m)(?s)/\*.*?\*/", COMMENT_MULTILINE),
            Rule::token(r"(?m)//.*", COMMENT_SINGLE),
            ...
            Rule::token_to(
                r"(?m)_?[A-Za-z][a-zA-Z0-9_-]*",
                NAME,
                NewState::Push(vec![r"extended_attribute_named_rest"]),
            ),
            Rule::token(r#"(?m)"[^"]*""#, STRING),
            Rule::token_to(
                r"(?m)\(",
                PUNCTUATION,
                NewState::Push(vec![r"identifier_list"]),
            ),
            Rule::default(NewState::Pop(1)),
        ],
    );
    m.insert(
        r"extended_attribute_named_rest",
        vec![
            Rule::token(r"(?m)\s+", TEXT),
            Rule::token(r"(?m)(?s)/\*.*?\*/", COMMENT_MULTILINE),
            Rule::token(r"(?m)//.*", COMMENT_SINGLE),
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)=",
                PUNCTUATION,
                NewState::Push(vec![r"default_value"]),
            ),
            Rule::default(NewState::Pop(1)),
        ],
    );
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)\)",
                PUNCTUATION,
                NewState::Push(vec![r"#pop", r"type_null"]),
            ),
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)\{",
                PUNCTUATION,
                NewState::Push(vec![r"namespace_body"]),
            ),
            Rule::token_to(r"(?m);", PUNCTUATION, NewState::Pop(1)),
        ],
    );
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)attribute(?![\w-])",
                KEYWORD,
                NewState::Push(vec![r"attribute_rest", r"type"]),
            ),
            Rule::token_to(
                r"(?m)const(?![\w-])",
                KEYWORD,
                NewState::Push(vec![r"const_rest", r"type"]),
            ),
            Rule::token_to(r"(?m)\}", PUNCTUATION, NewState::Pop(1)),
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)\{",
                PUNCTUATION,
                NewState::Push(vec![r"interface_body"]),
            ),
            Rule::token_to(r"(?m);", PUNCTUATION, NewState::Pop(1)),
        ],
    );
    ...
    m.insert(
        r"iterable_maplike_setlike_rest",
        vec![
            Rule::token(r"(?m)\s+", TEXT),
            Rule::token(r"(?m)(?s)/\*.*?\*/", COMMENT_MULTILINE),
            Rule::token(r"(?m)//.*", COMMENT_SINGLE),
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)=",
                PUNCTUATION,
                NewState::Push(vec![r"serialization_pattern"]),
            ),
            Rule::token_to(r"(?m);", PUNCTUATION, NewState::Pop(1)),
            Rule::default(NewState::Push(vec![r"operation"])),
        ],
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)\{",
                PUNCTUATION,
                NewState::Push(vec![r"serialization_pattern_map"]),
            ),
            Rule::token_to(
                r"(?m)\[",
                PUNCTUATION,
                NewState::Push(vec![r"serialization_pattern_list"]),
            ),
        ],
    );
    m.insert(
        r"serialization_pattern_map",
        vec![
            Rule::token(r"(?m)\s+", TEXT),
            Rule::token(r"(?m)(?s)/\*.*?\*/", COMMENT_MULTILINE),
            Rule::token(r"(?m)//.*", COMMENT_SINGLE),
    ...
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)interface(?![\w-])",
                KEYWORD,
                NewState::Push(vec![r"#pop", r"interface_rest"]),
            ),
            Rule::token(r"(?m)_?[A-Za-z][a-zA-Z0-9_-]*", NAME_CLASS),
            Rule::token_to(
                r"(?m)=",
                PUNCTUATION,
                NewState::Push(vec![r"operation", r"type"]),
            ),
            Rule::token_to(r"(?m);", PUNCTUATION, NewState::Pop(1)),
        ],
    );
    m.insert(
        ...
        vec![
            ...
            Rule::token_to(
                r"(?m)\{",
                PUNCTUATION,
                NewState::Push(vec![r"dictionary_body"]),
            ),
            Rule::token_to(r"(?m);", PUNCTUATION, NewState::Pop(1)),
        ],
    );
