...
impl TokenType {
    ...
    /// Short name used by the default HTML formatter's STANDARD_TYPES
    /// table — one or two letters per ancestor joined.
    /// Phase 0: simple lowercase-first-letter chain (full STANDARD_TYPES
    /// table lands with the Python lexer when fixtures demand it).
    pub fn short_name(&self) -> String {
        // Minimal placeholder — replaced in Phase 1 by the actual
        // pygments STANDARD_TYPES mapping.
        self.path
            .iter()
            .map(|seg| seg.chars().next().unwrap_or('?').to_ascii_lowercase())
            .collect()
    }
}
