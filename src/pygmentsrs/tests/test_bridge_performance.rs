...
mod bridge_performance_tests {
    ...
    fn perf_bridge_lex_large_file_100kb() {
        ...
        assert!(elapsed.as_secs() < 5, "lexing 100KB should be fast (took {:?})", elapsed);
    ...
    fn perf_bridge_lex_1mb_file() {
        ...
        assert!(elapsed.as_secs() < 30, "lexing 1MB should complete (took {:?})", elapsed);
    ...
    fn perf_bridge_lex_deeply_nested_50_levels() {
        ...
        eprintln!("Lexed 50-level nested JSON: {} tokens in {:?}", tokens.len(), elapsed);
    }
    ...
    fn perf_bridge_lex_deeply_nested_200_levels() {
        ...
        if let Some(tokens) = result {
            eprintln!("Lexed 200-level nested JSON: {} tokens in {:?}", tokens.len(), elapsed);
        } else {
    ...
    fn perf_bridge_format_to_all_formatters() {
        ...
        for formatter in formatters {
            ...
            if let Some(result) = bridge::format(formatter, &tokens) {
                let elapsed = start.elapsed();
                eprintln!("Format to {}: {:?} (output: {} bytes)", formatter, elapsed, result.len());
            }
        }
    }
    ...
    fn perf_bridge_repeated_lex_operations() {
        ...
        eprintln!("Repeated lex operations: first={:?}, last={:?}, avg={:?}", 
        ...
        assert!(last_time < max_acceptable, 
    ...
    fn perf_bridge_repeated_format_operations() {
        ...
        let avg_time = times.iter().sum::<std::time::Duration>() / times.len() as u32;
        eprintln!("Repeated format operations: avg={:?}, times={:?}", avg_time, times);
    ...
    fn perf_bridge_selection_analysis() {
        ...
        for (lexer, code, size_class) in test_cases {
            ...
            if let Some(tokens) = result {
                eprintln!("{:<12} | {} | {} tokens | {:?}", lexer, size_class, tokens.len(), bridge_time);
            }
        }
    ...
    fn perf_bridge_memory_scaling() {
        ...
        for size in sizes {
            ...
            if let Some(tokens) = bridge::lex("python", &code) {
                ...
                eprintln!("Input size: {} lines, Tokens: {}, Approx bytes: {}", 
                          size, tokens.len(), approx_memory);
            }
        }
    }
    ...
    fn perf_bridge_language_complexity() {
        ...
        for (lang, code) in code_samples {
            ...
            if let Some(tokens) = bridge::lex(lang, code) {
                ...
                eprintln!("{:<12}: {} tokens, avg {} bytes/token, {:?}", 
                          lang, tokens.len(), avg_token_len, elapsed);
            }
        }
    }
}
