...
fn run_pygments_with_stdin(args: &[&str], stdin: &str) -> (i32, String, String) {
    ...
    let output = child.wait_with_output().expect("Failed to wait for pygments");
    let code = output.status.code().unwrap_or(-1);
    let stdout = String::from_utf8_lossy(&output.stdout).to_string();
    let stderr = String::from_utf8_lossy(&output.stderr).to_string();
...
fn test_cli_list_lexers() {
    ...
    assert!(stdout.contains("Lexer") || stdout.contains("lexer"),
...
fn test_cli_list_formatters() {
    ...
    if code == 0 && !stdout.is_empty() {
        assert!(stdout.contains("Formatter") || stdout.contains("formatter") || stdout.contains("html"),
...
fn test_cli_highlight_stdin() {
    ...
    let (code, stdout, _stderr) = run_pygments_with_stdin(&["-l", "python", "-f", "html"], python_code);
...
fn test_cli_with_lexer() {
    ...
    assert!(stdout.len() >= python_code.len(), "Output should process the input");
}
...
fn test_cli_with_output_format() {
    ...
    let (code, stdout, _stderr) = run_pygments_with_stdin(&["-l", "python", "-f", "latex"], python_code);
...
fn test_cli_with_style() {
    ...
    let python_code = "x = 1";
    let (code, stdout, _stderr) = run_pygments_with_stdin(&["-l", "python", "-O", "style=monokai"], python_code);
...
fn test_cli_output_file() {
    ...
    assert!(!output_content.is_empty(), "Output file should contain content");
...
fn test_cli_invalid_lexer() {
    ...
    let (code, _stdout, stderr) = run_pygments_with_stdin(&["-l", "nosuchlexer_xyz_invalid"], "code");
    
    assert_ne!(code, 0, "Invalid lexer should exit with non-zero code");
    assert!(!stderr.is_empty() || code != 0, "Should have error or non-zero exit");
}
...
fn test_cli_nonexistent_file() {
    ...
    let (code, _stdout, _stderr) = run_pygments(&[
        "/nonexistent/path/that/does/not/exist_12345.py"
    ]);
...
fn test_cli_exit_code_success() {
    ...
    let python_code = "x = 42\ny = x + 1\nprint(y)";
    let (code, _stdout, _stderr) = run_pygments_with_stdin(&["-l", "python", "-f", "html"], python_code);
...
fn test_cli_multiple_lexer_formats() {
    ...
    for (lexer, format) in test_cases.iter().take(3) {
        let (code, stdout, _stderr) = run_pygments_with_stdin(
            &["-l", lexer, "-f", format],
            "x = 1",
        );
        ...
        if code == 0 {
            assert!(!stdout.is_empty(),
                "Should produce output for lexer={} format={}", lexer, format);
        }
    }
}
...
fn test_cli_large_input() {
    ...
    let (code, stdout, _stderr) = run_pygments_with_stdin(&["-l", "python", "-f", "html"], &large_code);
