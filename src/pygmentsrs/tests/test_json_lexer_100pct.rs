...
fn lex_repr(src: &str) -> Vec<(String, String)> {
    lex(src).into_iter().map(|(t, v)| (t.repr(), v)).collect()
}
...
fn string_simple() {
    ...
    assert!(
        result
            .iter()
            .any(|(t, v)| *t == STRING_DOUBLE && v == "\"hello\""),
...
fn string_with_space() {
    ...
    assert!(
        result.iter().any(|(t, _v)| *t == STRING_DOUBLE),
...
fn string_escape_simple() {
    ...
    assert!(
        result
            .iter()
            .any(|(t, v)| *t == STRING_DOUBLE && v.contains("\\n")),
...
fn string_unicode_escape_4_digits() {
    ...
    assert!(
        tokens
            .iter()
            .any(|(t, v)| t == "Token.Literal.String.Double" && v == "\"\\uffff\""),
...
fn string_unicode_followed_by_normal_escape() {
    ...
    let result = lex("\"\\u0041\\n\"");
    assert!(
        result.iter().any(|(t, _)| *t == STRING_DOUBLE),
...
fn whitespace_space() {
    let result = lex(" ");
    assert!(
        result.iter().any(|(t, _)| *t == WHITESPACE),
...
fn constant_true() {
    ...
    assert!(
        result
            .iter()
            .any(|(t, v)| *t == KEYWORD_CONSTANT && v == "true"),
...
fn constant_false() {
    ...
    assert!(
        result
            .iter()
            .any(|(t, v)| *t == KEYWORD_CONSTANT && v == "false"),
...
fn constant_null() {
    ...
    assert!(
        result
            .iter()
            .any(|(t, v)| *t == KEYWORD_CONSTANT && v == "null"),
...
fn constant_followed_by_punctuation() {
    ...
    assert!(
        result.iter().any(|(t, v)| *t == PUNCTUATION && v == "]"),
...
fn number_integer_zero() {
    let result = lex("0");
    assert!(
        result.iter().any(|(t, v)| *t == NUMBER_INTEGER && v == "0"),
...
fn number_integer_negative() {
    ...
    assert!(
        result
            .iter()
            .any(|(t, v)| *t == NUMBER_INTEGER && v == "-123"),
...
fn number_float_negative_exponent() {
    ...
    assert!(
        result
            .iter()
            .any(|(t, v)| *t == NUMBER_FLOAT && v == "2.5e-3"),
...
fn number_complex() {
    ...
    assert!(
        result.iter().any(|(t, _v)| *t == NUMBER_FLOAT),
...
fn number_float_at_eof() {
    let result = lex("3.14");
    assert!(
        result.iter().any(|(t, _)| *t == NUMBER_FLOAT),
...
fn comment_single_line() {
    ...
    assert!(
        result
            .iter()
            .any(|(t, v)| *t == COMMENT_SINGLE && v == "// comment"),
...
fn comment_single_line_with_content() {
    ...
    assert!(
        result.iter().any(|(t, _v)| *t == COMMENT_SINGLE),
...
fn comment_multiline_basic() {
    ...
    assert!(
        result
            .iter()
            .any(|(t, v)| *t == COMMENT_MULTILINE && v == "/* comment */"),
...
fn comment_multiline_nested_stars() {
    ...
    let result = lex("/* * comment */");
    assert!(
        result.iter().any(|(t, _)| *t == COMMENT_MULTILINE),
...
fn comment_single_before_eof() {
    ...
    let result = lex("// comment");
    assert!(
        result.iter().any(|(t, _)| *t == COMMENT_SINGLE),
...
fn colon_object_key_single_value() {
    ...
    assert!(
        result
            .iter()
            .any(|(t, v)| t == "Token.Name.Tag" && v == "\"key\""),
...
fn colon_nested_object() {
    ...
    assert!(
        result
            .iter()
            .any(|(t, v)| t == "Token.Name.Tag" && v == "\"outer\""),
        "Outer key not reclassified"
    );
    assert!(
        result
            .iter()
            .any(|(t, v)| t == "Token.Name.Tag" && v == "\"inner\""),
...
fn colon_with_non_string_before() {
    ...
    assert!(
        result.iter().any(|(t, v)| *t == NUMBER_INTEGER && v == "1"),
...
fn eof_number_integer() {
    ...
    let result = lex("42");
    assert!(
        result.iter().any(|(t, _)| *t == NUMBER_INTEGER),
...
fn eof_punctuation() {
    ...
    let result = lex("{}");
    assert!(
        result.iter().any(|(t, _)| *t == PUNCTUATION),
...
fn json_simple_object() {
    ...
    assert!(
        result.iter().any(|(t, _v)| t == "Token.Name.Tag"),
...
fn json_array_mixed_types() {
    ...
    assert!(
        result.iter().any(|(t, _)| *t == KEYWORD_CONSTANT),
        "Constant not found"
    );
}
...
fn json_scientific_notation() {
    ...
    let result = lex(src);
    assert!(
        result.iter().filter(|(t, _)| *t == NUMBER_FLOAT).count() >= 3,
...
fn edge_many_comments() {
    ...
    let result = lex(src);
    assert!(
        result.iter().filter(|(t, _)| *t == COMMENT_SINGLE).count() >= 3,
...
fn edge_only_whitespace() {
    let result = lex("   \n  \t  ");
    assert!(
        result.iter().all(|(t, _)| *t == WHITESPACE),
