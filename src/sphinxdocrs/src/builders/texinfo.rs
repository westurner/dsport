...
impl Builder for TexinfoBuilder {
    fn name(&self) -> &str { "texinfo" }
    fn format(&self) -> &str { "texinfo" }
    fn out_suffix(&self) -> &str { ".texi" }
    fn get_target_uri(&self, docname: &str) -> String { format!("{docname}.texi") }
    ...
    fn build_doc(&self, docname: &str, source: &str, outdir: &Path) -> Result<(), BuildError> {
        ...
        let output = format!("\\input texinfo\n@setfilename {title}.info\n@settitle {title}\n\n{body}\n@bye\n");
        let rel: PathBuf = docname.split('/').collect::<PathBuf>().with_extension("texi");
    ...
    fn build_all(&self, srcdir: &Path, outdir: &Path, env: &BuildEnvironment) -> Result<BuildResult, BuildError> {
        ...
        } else { env.all_docs.keys().cloned().collect() };
...
mod tests {
    ...
    fn writes_texinfo_header_and_body() {
        ...
        TexinfoBuilder::new().build_doc("index", "Title\n=====\n\nBody.\n", out.path()).unwrap();
