...
fn inventory_v2() -> Vec<u8> {
    dumps(
        ...
        "2.0",
        &[
            entry("module1", "py:module", "foo.html#module-module1", "Long Module desc"),
            entry("module2", "py:module", "foo.html#module-module2", "module2"),
            entry("module1.func", "py:function", "sub/foo.html#module1.func", "module1.func"),
            entry("CFunc", "c:function", "cfunc.html#CFunc", "CFunc"),
            entry("a term", "std:term", "glossary.html#term-a-term", "a term"),
            entry("The-Julia-Domain", "std:label", "write/latex.html#the-julia-domain", "The Julia Domain"),
            InventoryEntry {
                priority: -1,
...
fn v2_accepts_negative_priorities(inventory_v2: Vec<u8>) {
    let inv = Inventory::loads(&inventory_v2, "/util").unwrap();
    assert_eq!(inv.get("std:doc", "docname").unwrap().uri, "/util/docname.html");
}
...
fn v2_groups_objects_by_qualified_type(inventory_v2: Vec<u8>) {
    ...
    assert_eq!(
        types,
        ["c:function", "py:function", "py:module", "std:doc", "std:label", "std:term"]
    );
...
fn v2_keeps_the_first_of_duplicated_py_module_entries() {
    ...
    .unwrap();
    let inv = Inventory::loads(&raw, "/util").unwrap();
    assert_eq!(inv.get("py:module", "module1").unwrap().uri, "/util/first.html");
}
