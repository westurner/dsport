...
@pytest.mark.parametrize(
    "pkg,bin_name", [(pkg, bin_name) for pkg, bins in RUST_BINS for bin_name in bins]
)
def test_rust_binary_runs(pkg, bin_name):
    ...
    if result.returncode not in (0, 2):
        if "No module named" in output or "ModuleNotFoundError" in output:
            pytest.skip(
                f"{bin_name}: upstream Python module not installed ({output.strip()!r})"
            )
...
def test_python_script_runs(script):
    ...
    output = subprocess.check_output(
        [script, "--help"], text=True, stderr=subprocess.STDOUT
    )
